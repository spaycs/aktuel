...
import { View, Text, TextInput, TouchableOpacity, Modal, ScrollView, FlatList, Alert, SafeAreaView, Button } from 'react-native';
  ...
  const handleFetchKodlarList = async () => {
    try {
      if (defaults) {
        ...
        const apiUrl = firmaSenediValue === 'Firma Senedi' 
          ? `/Api/Kasa/Kasalar?firmano=${firmaNo}&tip=Verilen%20Senet%20Kasası` 
          : `/Api/Kasa/Kasalar?firmano=${firmaNo}&tip=Senet%20Kasası`;
        
        const response = await axiosLinkMain.get(apiUrl);
        if (firmaSenediValue === 'Firma Senedi') {
  ...
  {Platform.OS === 'ios' ? (
    ...
    >
      ...
      <Picker.Item label="Müşterisi" value="Müşterisi" style={MainStyles.textStyle} />
          ...
          {/* Tarih */}
          <Text style={MainStyles.formTitle}>Düzenleme Tarihi</Text>
...
*/}
            ...
            <TextInput
              ...
              onChangeText={setSck_banka_adres1}
            />
                  ...
                  renderItem={({ item }) => (
                    ...
                    >
                      <Text style={MainStyles.modalItemText}>
                        {item.Kod} - {item.Isim} - {item.Tip} 
                      </Text>
                    </TouchableOpacity>
                  )}
                />
                <TouchableOpacity style={{position :'absolute', marginTop: 12, marginLeft: 10}} onPress={() => setIsBankModalVisible(false)}>
...
export default TahsilatTediyeMusteriSenediModal;