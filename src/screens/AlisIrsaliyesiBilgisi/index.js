...
// Tüm Değişken Değerleri
  ...
  // Seçimler
  const [irsaliyeNormalTipi, setIrsaliyeNormalTipi] = useState('Alış');
  ...
  useEffect(() => {
    ...
    const logHareket = async () => {
      ...
      try {
        ...
        const body = {
          Message: 'Alış İrsaliye Sayfa Açıldı', // Hardcoded message
          User: defaults[0].IQ_MikroPersKod, // Temsilci ID
          database: defaults[0].IQ_Database, // Database ID
          data: 'Alış İrsaliyesi' // Hardcoded data
        };
...
// Tüm Değişken Değerleri
  ...
  // Değiştirilebilir Alanlar 
    useEffect(() => {
      if (defaults && defaults[0]) {
        const { IQ_AlisIrsaliyeSeriNoDegistirebilir, IQ_CikisDepoNoDegistirebilir, IQ_VadePasifGelsin } = defaults[0];
        setIsEditable(IQ_AlisIrsaliyeSeriNoDegistirebilir === 1);
  ...
  // Sayfa Açıldığında Gönderilen Varsayılan Değerler
    ...
    }, [sth_evrakno_seri,sth_evrakno_sira, sth_giris_depo_no, sth_cikis_depo_no, ] );
  // Sayfa Açıldığında Gönderilen Varsayılan Değerler
  ...
  useEffect(() => {
    fetchDovizList();
    fetchDepoList();
    handleIrsaliyeTipiChange('Toptan Alış');
    handleIrsaliyeNormalTipiChange('Alış');
  }, []);
  ...
  // İrsaliye Tipi Varsayılan Seçim
    const handleIrsaliyeTipiChange = (itemValue) => {
      ...
      let sth_tip = 0;
      let sth_cins = 0;
      let sth_normal_iade = faturaBilgileri.sth_normal_iade;
    ...
    const handleIrsaliyeNormalTipiChange = (itemValue) => {
      ...
      switch (itemValue) {
        case 'Alış':
          sth_normal_iade = 0;
          setPickerOptions([
            { label: 'Toptan Alış İrsaliyesi', value: 'Toptan Alış' },
            { label: 'Perakende Alış İrsaliyesi', value: 'Perakende Alış' },
            { label: 'İhraç Kayıtlı Alış İrsaliyesi', value: 'İhraç Kayıtlı Alış' },
            { label: 'İthalat Alış İrsaliyesi', value: 'İthalat Alış' },
          ]);
          break;
        case 'İade':
          sth_normal_iade = 1;
          setPickerOptions([
            { label: 'Toptan İade Alış İrsaliyesi', value: 'Toptan İade' },
            { label: 'Perakende İade Alış İrsaliyesi', value: 'Perakende İade' },
            { label: 'İhraç Kayıtlı İade Alış İrsaliyesi', value: 'İhraç Kayıtlı İade' },
            { label: 'İthalat İade Alış İrsaliyesi', value: 'İthalat İade Alış' },
          ]);
          break;
        default:
          setPickerOptions([]);
      ...
      <Text style={MainStyles.formTitle}>Evrak</Text> 
        ...
        {Platform.OS === 'ios' ? (
              ...
              <View style={MainStyles.modalContainerPicker}>
                <View style={MainStyles.modalContentPicker}>
                  ...
                  >
                    ...
                    <Picker.Item label="Alış" value="Alış" />
          ...
          >
            ...
            <Picker.Item label="Alış" value="Alış" style={MainStyles.textStyle} />
...
export default AlisIrsaliyesiBilgisi;
