...
const CekSenetListesi = () => {
   ...
   useEffect(() => {
     ...
     const logHareket = async () => {
       ...
       try {
         ...
         const body = {
           Message: 'Çek Senet Listesi Rapor Açıldı', // Hardcoded message
           User: defaults[0].IQ_MikroPersKod, // Temsilci ID
           database: defaults[0].IQ_Database, // Database ID
           data: 'Çek Senet Listesi Rapor' // Hardcoded data
         };
  ...
  const fetchData = async () => {
    ...
    setError('');
    try {
      const response = await axiosLinkMain.get(`/Api/Raporlar/CekSenetListesi?ilktarih=${formatDateForApi(startDate)}&sontarih=${formatDateForApi(endDate)}`);
