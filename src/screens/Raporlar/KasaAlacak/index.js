...
const KasaAlacak = () => {
   ...
   useEffect(() => {
     ...
     const logHareket = async () => {
       ...
       try {
         ...
         const body = {
           Message: 'Kasa Alacak Rapor Açıldı', // Hardcoded message
           User: defaults[0].Adi, // Temsilci ID
           database: defaults[0].IQ_Database, // Database ID
           data: 'Kasa Alacak Rapor' // Hardcoded data
         };
  ...
  const fetchData = async () => {
    ...
    setError('');
    try {
      const response = await axiosLinkMain.get(`/Api/Raporlar/KasaRaporuAlacak?ilktarih=${formatDateForApi(startDate)}&sontarih=${formatDateForApi(endDate)}`);
