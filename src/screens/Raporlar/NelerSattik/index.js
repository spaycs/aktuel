...
const NelerSattik = () => {
   ...
   useEffect(() => {
     ...
     const logHareket = async () => {
       ...
       try {
         ...
         const body = {
           Message: 'Neler Sattık Rapor Açıldı', // Hardcoded message
           User: defaults[0].Adi, // Temsilci ID
           database: defaults[0].IQ_Database, // Database ID
           data: 'Neler Sattık Rapor' // Hardcoded data
         };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axiosLinkMain.get(`/Api/Raporlar/NelerSattik?cari=${cariKodu}&ilktarih=${formatDateForApi(startDate)}&sontarih=${formatDateForApi(endDate)}`);
